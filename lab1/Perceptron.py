...
class Perceptron:
    ...
    def initializeWeights(self, size, output_size):
        self.weights = [[random.random() * random.choice([-1, 1]) for num in range(size + 1)] for output in
                        range(output_size)]
    ...
    def adjustWeights(self, input_, output_node, possible_output, error):
        ...
        for weight in range(len(self.weights[output_node])):
            ...
            self.weights[output_node][weight] = old_weight + (
            dWeight_without_x * input_[possible_output]['data'][weight])
